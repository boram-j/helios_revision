...
int main(int argc, char *argv[])
{
    if (argc != 9)
    {
        printf("./tpch8 < # runs > < debug mode > <# LINEITEM> < # PART > <numTrhead>\
...
#pragma omp single
{
        ...
        #pragma omp task
        {   
            tmp1 = GTE(comparator, evaluator, relin_keys,
                                ctxt_o_orderdate, tgt_date1);
        }
        #pragma omp task
        {
            tmp3 = GTE(comparator, evaluator, relin_keys,
                                ctxt_o_orderdate, tgt_date3);
        }
        #pragma omp task
        {
            tmp2 = LTE(comparator, evaluator, relin_keys,
        ...
        #pragma omp taskwait
        for(size_t i = 1; i < ctxt_joined_partkey.size(); i++) 
            ...
            for(int i = 0; i < NUM_REGION; i++) {
                ...
                mask_r[i] = 1;
                batch_encoder.encode(mask_r, ptxt_mask);
                // BATCH -> SINGLE
        ...
        #pragma omp taskwait
            ...
            for(int i = 0; i < NUM_NATION; i++) {
                ...
                mask_n[i] = 1;
                batch_encoder.encode(mask_n, ptxt_mask);
                // BATCH -> SINGLE
        ...
        #pragma omp taskwait
            ...
            for(int i = 0; i < numEle_c; i++) {
                ...
                mask_c[i] = 1;
                batch_encoder.encode(mask_c, ptxt_mask);
                // BATCH -> SINGLE
        ...
        #pragma omp taskwait
            for(size_t i = 1; i < ctxt_joined_custkey_.size(); i++) 
            ...
            for(int i = 0; i < numEle_o; i++) {
                ...
                mask_o[i] = 1;
                batch_encoder.encode(mask_o, ptxt_mask);
            ...
            cout << "    + noise budget left: " << decryptor.invariant_noise_budget(ctxt_joined_orderkey_[1][0]) << " bits"
         << endl;
            ...
            for(int i = 0; i < numEle_s; i++) {
                ...
                mask_s[i] = 1;
                batch_encoder.encode(mask_s, ptxt_mask);
                // BATCH -> SINGLE
            ...
            for(size_t i = 1; i < ctxt_joined_supkey_.size(); i++) 
        ...
        evaluator.multiply(ctxt_joined_supkey, ctxt_total_vol95, ctxt_mkt_share95);
        evaluator.relinearize_inplace(ctxt_mkt_share95, relin_keys);
        evaluator.multiply(ctxt_joined_supkey, ctxt_total_vol96, ctxt_mkt_share96);        
