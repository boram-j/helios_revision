...
void verify(vector<int64_t>& o_orderpriority, vector<int64_t>& o_orderdate, 
    ...
    for(size_t i = 0; i < o_orderdate.size(); i++) {
...
void verify_filtered_ordertable(vector<int64_t> &o_orderpriority, vector<int64_t> &o_orderdate,
    ...
    for (int i = 0; i < NUM_PRIORITY_GROUP; i++) {
        vector<int64_t> decrypted = print_dec_<int64_t>("verify_filtered_ordertable", 
                                    ...
                                    batch_encoder, decryptor);
        for(size_t j = 0; j < o_orderdate.size(); j++) {
            if (group[i][j]!= decrypted[j]){
                printf("[%d][%ld] Expected: %ld Result: %ld\n", 
...
void verify_exist(vector<int64_t> &l_commitdate, vector<int64_t> &l_receiptdate,
    ...
    for(size_t i = 0; i < l_commitdate.size(); i++) {
        ...
        if(cond != decrypted[i]) {
            printf("[%ld] Expected: %d Result: %ld\n", 
                    i, cond, decrypted[i]);
            exit(1);
        }
    }
   ...
   for(int k = 0; k < runs; k++)
    {
        ...
        Plaintext ptxt_l_orderkey, ptxt_l_commitdate, ptxt_l_receiptdate;
        Ciphertext ctxt_o_orderpriority, ctxt_o_orderkey, ctxt_o_orderdate;
        Ciphertext ctxt_l_orderkey, ctxt_l_commitdate, ctxt_l_receiptdate;
        ...
        ctxt_grp_res = GROUPBY(comparator, evaluator, relin_keys, ctxt_o_orderpriority, ptxt_priority_group);
        ...
        if(debug == 2) {
            verify_filtered_ordertable(o_orderpriority, o_orderdate, ctxt_grp_res, batch_encoder, decryptor);
        }
        ...
        for(int p = 0; p < NUM_PRIORITY_GROUP; p++) {
            ...
            for (int i = 0; i < numEle_o; i++) {
                ...
                ctxt_key_match = comparator.isEqual(evaluator, relin_keys, ctxt_l_orderkey, ptxt_orderkey);
                evaluator.multiply(ctxt_lt_receiptdate, ctxt_key_match, ctxt_exist);
                evaluator.relinearize_inplace(ctxt_exist, relin_keys);
                time_filter.pause();
                
                if(debug == 2) {
                    verify_exist(l_commitdate, l_receiptdate, l_orderkey, i+1, ctxt_exist, batch_encoder, decryptor);
                }
                ...
                ctxt_ordertable = SUM(evaluator, ctxt_ordertable, slot_count, galois_keys); //BATCH -> SINGLE
            ...
            time_agg.start();
            ctxt_order_count[p] = SUM(evaluator, ctxt_order_count[p], slot_count, galois_keys);
            time_agg.pause();
        }
        ...
        time_tpch.pause();
        cout << "    + noise budget left: " << decryptor.invariant_noise_budget(ctxt_order_count[0]) << " bits" << endl;
        
        if(debug != 0) {
            verify(o_orderpriority, o_orderdate, l_orderkey, o_orderkey, l_commitdate, l_receiptdate, ctxt_order_count, batch_encoder, decryptor);
        }
    }
    time_tpch.stop(runs);
    time_agg.stop(runs);
