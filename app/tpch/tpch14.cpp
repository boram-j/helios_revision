...
template <typename T>
inline void print_vec(std::string st, const std::vector<T> &vec, size_t numEle) {
    std::cout << st << ":" << std::setw(3) << "[";
    for (size_t i = 0; i < numEle && i < vec.size(); i++)
    {
...
// ==============================================================================
// Verification Logic (TPC-H 14 Specific)
// ==============================================================================
...
int main(int argc, char *argv[])
{
    if (argc != 5)
    {
        printf("./tpch14 < # runs > < debug mode > < # numEle > < # numEle for Part>\n"
            "[Debug Mode]\n0 - no debug\n"
   ...
   for(int k = 0; k < runs; k++)
    {
        ...
        */
        ...
        Ciphertext ctxt_joined = ctxt_key_match[0];
        ...
        evaluator.relinearize_inplace(ctxt_filter, relin_keys);
        Ciphertext ctxt_promo = SUM(evaluator, ctxt_joined, slot_count, galois_keys);
        Ciphertext ctxt_total = SUM(evaluator, ctxt_filter, slot_count, galois_keys);
        time_agg.pause();

        auto context_data = context.first_context_data();
        while (context_data->next_context_data())
        {
            evaluator.mod_switch_to_next_inplace(ctxt_promo);
            evaluator.mod_switch_to_next_inplace(ctxt_total);
        ...
        cout << "    + noise budget left: " << decryptor.invariant_noise_budget(ctxt_promo) << " bits"
        << endl;
    }
