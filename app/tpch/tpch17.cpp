...
// ==============================================================================
// Verification Logic (TPC-H 17 Specific)
// ==============================================================================
...
void verify(vector<int64_t> l_partkey, vector<int64_t> l_extendedprice, vector<int64_t> l_quantity, 
    ...
    int64_t res_cnt = print_dec_<int64_t>("SUM_CNT", ctxt_cnt, l_partkey.size(), 
                                               batch_encoder, decryptor)[0];
    if(cnt != res_cnt) {
        printf("SUM_CNT Expected: %ld Result: %ld\n", cnt, res_cnt);
        exit(1);
    }
    if(sum != res_sum) {
        printf("SUM_YEARLY Expected: %ld Result: %ld\n", sum, res_sum);
        exit(1);
    }
}
...
int main(int argc, char *argv[])
{
    if (argc != 5)
    {
        printf("./tpch17 < # runs > < debug mode > < # numEle > < # numEle for Part>\n"
            "[Debug Mode]\n0 - no debug\n"
   ...
   for(int k = 0; k < runs; k++)
    {
        ...
        */
        ...
        {
            evaluator.mod_switch_to_next_inplace(ctxt_cnt);
            evaluator.mod_switch_to_next_inplace(ctxt_sum_yearly);
